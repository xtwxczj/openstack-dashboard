...
class ConsoleTab(tabs.Tab):
    ...
    def get_context_data(self, request):
        ...
        # Check image property before proceeding.
        image = api.glance.image_get(request, instance.image['id'])
        if not image or image.name.startswith('UI'):
           #or not hasattr(image, 'properties') or \
           #'has_shared_disk' not in image.properties.keys() or \
           #image.properties.get('has_shared_disk') != 'False':
            return {'console_url': None, 'instance_id': instance.id}
